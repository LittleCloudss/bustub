...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *now = &this->root_;
    std::unique_ptr<TrieNode> *father = now;
    for (char i : key) {
      father = now;
      now = (*now)->GetChildNode(i);
      if (now == nullptr) {
        now = (*father)->InsertChildNode(i, std::make_unique<TrieNode>(i));
      }
    }
    if ((*now)->IsEndNode()) {
    ...
    (*father)->InsertChildNode(last_key, std::move(tmp));
    this->latch_.WUnlock();
    return true;
  }
   ...
   */
  void RemoveNode(const std::string &key, std::unique_ptr<TrieNode> *now, size_t depth) {
    if (depth == key.length()) {
      (*now)->SetEndNode(false);
      return;
    }
    std::unique_ptr<TrieNode> *tmp = (*now)->GetChildNode(key[depth]);
    RemoveNode(key, tmp, depth + 1);
    if (!(*tmp)->HasChildren() && !(*tmp)->IsEndNode()) {
      (*now)->RemoveChildNode(key[depth]);
    }
  }
  bool Remove(const std::string &key) {
    ...
    if (!(*now)->IsEndNode()) {
      // LOG_DEBUG("Remove: key %s is not a endnode!\n", key.c_str());
      this->latch_.WUnlock();
      return false;
    }
    RemoveNode(key, &this->root_, 0);
    this->latch_.WUnlock();
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto tmp = dynamic_cast<TrieNodeWithValue<T> *>((*now).get());
    if (tmp == nullptr) {
      *success = false;
      this->latch_.RUnlock();
      return {};
